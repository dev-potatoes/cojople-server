...
public class AuthenticationConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        return httpSecurity
                ...
                .authorizeRequests()//request를 authorized하겠다.
//                .antMatchers("/api/**").permitAll()
                .antMatchers("/api/v1/users/join","/api/v1/users/login").permitAll() //join, login은 언제나 허용
                .antMatchers(HttpMethod.POST, "/api/v1/**").authenticated() //join, login을 제외한 모든 POST요청을 인증필요로 막아둠
                .and()
                .sessionManagement()
                ...
                //username하고 password가지고 로그인할 때 인증을 했기 때문에 서버에서 토큰을 준건데 token으로 인증을 할 것이기 때문에 추가
